...
> 对外工作品牌：**叠象 Stelora · 万能文件工作台**。
> `叠象` / `Stelora` 尚待商标、商号和域名正式核验；核验前仓库、CLI、crate 和 Action ID
> 继续使用 Redline，避免营销命名破坏技术兼容。

**面向 AI 时代的模型中立万能文件工作台。**
...
Claude、Codex 和本地模型不是竞争对手，而是可替换的发动机。Redline 负责系统入口、
格式、标注、不可变版本、差异和审计；模型负责理解与生成。模型越强，工作台越强。
...
| 想了解 | 读这里 |
|---|---|
| 品牌、品类、用户与竞争策略 | [产品战略](./docs/产品战略.md) |
| 免费层、收入来源与商业原则 | [商业模型](./docs/商业模型.md) |
| Apache-2.0、私有服务端和商标边界 | [开源与商业化](./docs/开源与商业化.md) |
| 产品能力如何映射到 Action 和代码 | [功能与代码规划](./docs/功能与代码规划.md) |
| 传统文件如何映射成 AI 可计算的视觉/语义影子 | [影文档协议](./docs/影文档协议.md) |
| ShadowDoc 怎样发布为开放规范并形成生态 | [ShadowDoc 开源规范与生态计划](./docs/ShadowDoc开源规范与生态计划.md) |
| 当前任务、优先级与验收标准 | [开发计划](./docs/开发计划.md) |
