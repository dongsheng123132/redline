...
export default function OfficeOutlineViewer({
  ...
  useEffect(() => {
    ...
    if (!isPptx) {
      ...
      onUnitsResolved([{ id: "0", index: 0, label: "旧版二进制格式" }]);
      setSlides([]);
      return;
    }
    (async () => {
      try {
        ...
        const list = slideNames.map((name, i) => {
          ...
          const text = Array.from(xml.matchAll(/<a:t>([^<]*)<\/a:t>/g))
            ...
            .join(" ")
            .trim();
          return { label: `第 ${i + 1} 页`, text: text || "（无文字内容）" };
        });
        setSlides(list);
  ...
  }, [doc, bytes, onUnitsResolved]);

  if (err) {
    return (
      <div className="p-4 text-red-400 text-[12px]">大纲提取失败：{err}（建议"用默认程序打开"）</div>
    );
  }
  ...
  if (slides && slides.length === 0) {
    return (
      <div className="p-4 flex flex-col gap-2 text-[12px]">
        <div className="text-gray-400">旧版二进制格式（.ppt/.doc/.xls）暂不支持预览。</div>
        {openExternal && (
          <button
  ...
  return (
    <div className="relative flex-1 overflow-auto min-h-0 p-4">
      ...
      {activeUnitId && (
        <div className="absolute inset-0">{renderOverlay(activeUnitId, { width: 800, height: 600 })}</div>
      )}
    </div>
  );
}
