...
/// 跑一次 CLI，返回退出码。**不自己 exit** —— 调用方（独立 CLI 二进制、或者
/// 合并后的单 exe）决定怎么收尾。
pub fn run() -> i32 {
    let cli = Cli::parse();
    ...
    if ok {
        ...
        exit_code_of(&envelope)
    }
}

/// 全部子命令名。合并成单 exe 之后要靠它区分「这是 CLI 调用」还是
...
pub fn is_cli_invocation(args: &[String]) -> bool {
    args.first().is_some_and(|first| SUBCOMMANDS.contains(&first.as_str()))
}
